defmodule ExLearning.Axon.MnistGAN do
  require Axon
  ...
  def build_discriminator(input_shape) do
  ...
  def run(discriminator, generator, train_images) do
    discriminator
  ...
  defp log_iteration(state) do
    ...
    g_loss = "G: #{:io_lib.format(~c"~.5f", [Nx.to_number(pstate[:generator][:loss])])}"
    d_loss = "D: #{:io_lib.format(~c"~.5f", [Nx.to_number(pstate[:discriminator][:loss])])}"
